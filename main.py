...
from src import BO_NE, check_resutls_dir, RPS, hotelling, Saddle, BudgetAllocation, Saddle_var
...
ACQS = ['pred', 'sur', 'epsilon_greedy', 'ballet']
SUBSAMPLE_NUM = 2000
TASK_SAMPLE_NUM = {
    'rps': SUBSAMPLE_NUM,
    'saddle': 1000,
...
def main(task:str='RPS', acq_method:str='ts', retrain_interval:int=10, n_repeat:int=10, noisy_obs:bool=False,
         opt_steps:int=10, train_iter:int=100, n_init:int=10, lr:float=1e-4, interpolate:bool=False, 
         subsample:bool=False, spectral_norm:bool=False, gp_type:str='dk', beta:float=1, verbose:bool=False,
         **kwargs)->np.ndarray:
    '''
    Inputs:
        @task: str, the task to run, one of ['RPS', 'hotelling', 'saddle']
        @acq_method: str, the acquisition function to use, one of [ 'pred', 'sur', 'epsilon_greedy']
    ...
    elif task.lower() == 'hotelling':
        problem = hotelling()

    elif task.lower() == 'saddle':
        problem = Saddle()
    
    elif task.lower() == 'saddle_var':
        problem = Saddle_var()
    ...
    init_y_list = [torch.from_numpy(init_y[:, 0]), torch.from_numpy(init_y[:, 1])]
    ...
    for rep in tqdm.auto.tqdm(range(n_rep), desc=f'method {acq_method}'):
        ...
        for idx in iterator:
            with warnings.catch_warnings():
                ...
                if subsample:
                    candidate = bo.query(test_x, acq=acq_method, subsample_num=TASK_SAMPLE_NUM.get(task.lower(), SUBSAMPLE_NUM), beta=beta, **kwargs)
                else:
                    candidate = bo.query(test_x, acq=acq_method, beta=beta, **kwargs)
                models = bo._models

            _new_x = candidate.numpy()
            _new_y_1, _new_y_2 = problem.query_utilities(_new_x)
            ...
            if not noisy_obs:
                ...
                init_y_list[0] = torch.cat([init_y_list[0], torch.from_numpy(_new_y_1.reshape(1)) + torch.randn(1) * 1e-2], dim=0)
                init_y_list[1] = torch.cat([init_y_list[1], torch.from_numpy(_new_y_2.reshape(1)) + torch.randn(1) * 1e-2], dim=0)
...
if __name__ == '__main__':
    ...
    args.add_argument('--task', type=str, default='saddle')
    args.add_argument('--acq_method', type=str, default='ballet')
    args.add_argument('--lr', type=float, default=1e-4)
    args.add_argument('--beta', type=float, default=2)
    args.add_argument('--global_search', action='store_true') 
    args.add_argument('--plot_result', action='store_true')
    ...
    args.add_argument('--dir', type=str, default='./results')
    ...
    regret = main(task=task, acq_method=f'{acq_method}', train_iter=train_iter,  n_init=n_init, 
                    retrain_interval=retrain_interval, opt_steps=opt_steps, n_repeat=n_repeat, 
                    lr=lr, interpolate=interpolate, on_roi=on_roi, spectral_norm=args.spectrum_norm, 
                    subsample=subsample, gp_type=gp_type, beta=beta, verbose=args.verbose, noisy_obs=args.noise)    
    ...
    target_dir = f"{args.dir}/{task.upper()}_{acq_method.lower()}_{gp_type}_SimReg_B{beta}_R{n_repeat}_OS{opt_steps}_TI{train_iter}_RI{retrain_interval}_NI{args.n_init}{'_IP' if interpolate else ''}{'_SS' if subsample else''}_{time_stamp}_{args.name}{'_roi' if on_roi else ''}{'_noisy' if noise else ''}.p"
